Here are some useful commands for managing a Red Hat-based system (such as **RHEL** or **CentOS**), including system administration, networking, package management, and more:

### **System Information**
1. **Check system information**:
   - `uname -a` — Displays system information (kernel, architecture, etc.).
   - `hostname` — Displays the system’s hostname.
   - `cat /etc/redhat-release` — Displays the Red Hat version.
   - `top` — Displays CPU and memory usage.
   - `htop` — Enhanced process viewer (needs to be installed: `sudo yum install htop`).
   - `free -h` — Displays memory usage.
   - `df -h` — Displays disk space usage.
   - `lscpu` — Displays CPU architecture and info.
   - `lsblk` — Lists block devices (disks and partitions).

### **Package Management**
Red Hat uses **`dnf`** (or `yum` on older versions) for package management.

1. **Update package list**:
   - `sudo dnf check-update` — Checks for updates without installing them.
2. **Install a package**:
   - `sudo dnf install <package_name>` — Installs a package.
3. **Remove a package**:
   - `sudo dnf remove <package_name>` — Removes a package.
4. **Upgrade all packages**:
   - `sudo dnf upgrade` — Upgrades all installed packages.
5. **Search for a package**:
   - `dnf search <package_name>` — Searches for a package in the repositories.
6. **Show package details**:
   - `dnf info <package_name>` — Displays detailed info about a package.
7. **List installed packages**:
   - `dnf list installed` — Lists all installed packages.
8. **List available updates**:
   - `dnf updateinfo` — Displays available updates and security advisories.

### **File and Directory Operations**
1. **List files**:
   - `ls` — Lists files in the current directory.
   - `ls -l` — Lists files with detailed information.
   - `ls -a` — Lists all files, including hidden files.
2. **Change directory**:
   - `cd <directory>` — Changes to the specified directory.
   - `cd ..` — Moves one directory up.
   - `cd ~` — Moves to the home directory.
3. **Create a directory**:
   - `mkdir <directory_name>` — Creates a new directory.
4. **Remove a file**:
   - `rm <file_name>` — Removes a file.
   - `rm -rf <directory_name>` — Removes a directory and its contents recursively.
5. **Copy a file**:
   - `cp <source> <destination>` — Copies a file or directory.
6. **Move/Rename a file**:
   - `mv <source> <destination>` — Moves or renames a file.
7. **View file contents**:
   - `cat <file_name>` — Displays the content of a file.
   - `less <file_name>` — View file content with scroll support.
   - `tail -f <file_name>` — View the end of a file in real-time (great for logs).

### **File Permissions and Ownership**
1. **Change file permissions**:
   - `chmod <permissions> <file_name>` — Changes file permissions (e.g., `chmod 755 <file_name>`).
2. **Change file ownership**:
   - `chown <user>:<group> <file_name>` — Changes ownership of a file.

### **Process Management**
1. **List running processes**:
   - `ps aux` — Lists all running processes.
   - `top` — Interactive process viewer.
2. **Kill a process**:
   - `kill <pid>` — Terminates a process by its process ID (PID).
   - `kill -9 <pid>` — Forcefully kills a process.
3. **Find a process**:
   - `pgrep <process_name>` — Finds the PID of a process.
4. **Show system resource usage**:
   - `uptime` — Displays system uptime, load averages, and current time.
   - `vmstat` — Displays system performance stats (memory, CPU, disk).

### **Networking**
1. **Check IP address**:
   - `ip a` or `ifconfig` — Displays network interface information.
2. **Ping a host**:
   - `ping <hostname_or_ip>` — Pings a host to check connectivity.
3. **Check open ports**:
   - `sudo ss -tuln` — Displays open ports and associated services.
   - `sudo netstat -tuln` — Lists open ports (older versions of RHEL may use `netstat`).
4. **Traceroute**:
   - `traceroute <hostname_or_ip>` — Traces the route packets take to a network host.
   - (Install if not available: `sudo dnf install traceroute`).

### **User and Group Management**
1. **Add a user**:
   - `sudo useradd <username>` — Adds a new user.
2. **Add a group**:
   - `sudo groupadd <groupname>` — Adds a new group.
3. **Add user to group**:
   - `sudo usermod -aG <groupname> <username>` — Adds a user to a group.
4. **Delete a user**:
   - `sudo userdel <username>` — Deletes a user.
   - `sudo userdel -r <username>` — Deletes a user and their home directory.
5. **Switch user**:
   - `su <username>` — Switches to another user.
   - `sudo -u <username> <command>` — Runs a command as another user.

### **Disk Management**
1. **Check disk usage**:
   - `df -h` — Displays disk space usage.
2. **Check disk partitions**:
   - `sudo fdisk -l` — Lists all disk partitions.
3. **Create a new partition**:
   - `sudo fdisk /dev/sda` — Create partitions on `/dev/sda` disk.
4. **Mount a disk**:
   - `sudo mount <device> <mount_point>` — Mounts a filesystem.
5. **Unmount a disk**:
   - `sudo umount <mount_point>` — Unmounts a filesystem.
6. **Format a disk**:
   - `sudo mkfs.ext4 <device>` — Formats a disk with the ext4 filesystem.

### **System Services (systemd)**
1. **Check status of a service**:
   - `sudo systemctl status <service_name>` — Displays the status of a service.
2. **Start/Stop/Restart a service**:
   - `sudo systemctl start <service_name>` — Starts a service.
   - `sudo systemctl stop <service_name>` — Stops a service.
   - `sudo systemctl restart <service_name>` — Restarts a service.
3. **Enable/Disable a service at boot**:
   - `sudo systemctl enable <service_name>` — Enables a service to start on boot.
   - `sudo systemctl disable <service_name>` — Disables a service from starting on boot.
4. **List all services**:
   - `sudo systemctl list-units --type=service` — Lists active services.

### **System Shutdown/Restart**
1. **Shutdown system**:
   - `sudo shutdown now` — Immediately shuts down the system.
   - `sudo shutdown -h +10` — Shuts down the system in 10 minutes.
2. **Restart system**:
   - `sudo reboot` — Restarts the system.
3. **Power off**:
   - `sudo poweroff` — Powers off the system.

### **Miscellaneous**
1. **Search for a file**:
   - `find /path/to/search -name <filename>` — Searches for a file by name.
2. **Search command history**:
   - `history | grep <command>` — Searches through the command history.
3. **Show disk usage by directory**:
   - `du -sh <directory>` — Shows the total disk usage of a directory.
4. **Clear terminal screen**:
   - `clear` — Clears the terminal screen.
5. **Check last login**:
   - `last` — Shows the last login information for users.

### **SELinux (Security-Enhanced Linux)**
1. **Check SELinux status**:
   - `sestatus` — Displays the current status of SELinux (enabled/disabled).
2. **Set SELinux mode**:
   - `sudo setenforce 0` — Set SELinux to permissive mode.
   - `sudo setenforce 1` — Set SELinux to enforcing mode.
3. **Change SELinux policy**:
   - `sudo semanage boolean -l` — Lists SELinux boolean settings.
   - `sudo semanage boolean --modify --on <boolean>` — Modify SELinux boolean settings.

---

These commands are essential for daily management of Red Hat-based systems. Let me know if you'd like more details or if you want to explore any specific command further!
